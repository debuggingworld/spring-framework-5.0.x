...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		// name 有可能是 &xxx 或者 xxxx，如果是 &xxx，那么 beanNane 就是 xxx
		// name 有可能是 别名，那么 beanName 就是 id
		...
		if (sharedInstance != null && args == null) {
			...
			// 根据 name 返回 Bean 或者 factoryBean
			...
			if (parentBeanFactory != null && !containsBeanDefinition(beanName)) {
				// 当前 BeanFactory 中没有 BeanDefinition，则去父 BeanFactory 中去找
			...
			try {
				...
				// 检查 BeanDefinition 是不是 抽象的
				checkMergedBeanDefinition(mbd, beanName, args);

				// Guarantee initialization of beans that the current bean depends on.
				String[] dependsOn = mbd.getDependsOn();
				if (dependsOn != null) {
					// 当前 Bean 创建之前，依赖的 Bean 必须已经创建完成
	...
	public boolean isTypeMatch(String name, ResolvableType typeToMatch) throws NoSuchBeanDefinitionException {
		...
		// 判断 name 所有对应的 Bean 类型是不是 typeToMatch

		// 如果name是&xxx,则 beanName是xxx
		String beanName = transformedBeanName(name);
		...
		// 单例池中不存在，就根据 BeanDefinition 来判断
	...
	protected Object getObjectForBeanInstance(
		...
		if (object == null) {
			...
			// 调用 FactoryBean 的 getObject() 创建并加入缓存
