...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	protected boolean isAutowireCandidate(
		...
		// 根据 BeanDefinition 判断 beanName 对应的 Bean 可不可以用来进行依赖注入
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		// 实例化非懒加载单例 Bean
		for (String beanName : beanNames) {
			// 获取合并后的 BeanDefinition ( 从父 BeanDefinition 获取自己未定义的属性）
			// 合并 BeanDefinition 将生成一个新的 RootBeanDefinition
			// 没有父 BeanDefinition 将属性复制到一个新的 RootBeanDefinition
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
			// 不是抽象的 BeanDefinition &&  单例非懒加载
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				if (isFactoryBean(beanName)) {
					// 获取 FactoryBean 对象
					// 一个 FactoryBean 对应一个 BeanDefinition 和两个 Bean
					// 实例化 FactoryBean 本身 Bean ( 缓存在 singletonObjects )
					// 如果实现 SmartFactoryBean 并且 isEagerInit() 返回 true，则同时实例化 FactoryBean ( 缓存在 factoryBeanObjectCache)
		...
		// 所有非懒加载 Bean 实例化后调用 SmartInitializingSingleton.afterSingletonsInstantiated()
		for (String beanName : beanNames) {
			// 从 singletonFactories  到  earlySingletonObjects
	...
	public Object resolveDependency(DependencyDescriptor descriptor, @Nullable String requestingBeanName,
		...
		// 初始化入参获取器，为了获取方法参数的类型和变量名（1.8 反射和本地变量表）
		...
		if (Optional.class == descriptor.getDependencyType()) {
			...
			// 在属性或 set 方法上使用 @Lazy 注解，构造一个代理对象返回
			// 没有 @Lazy 返回空
			Object result = getAutowireCandidateResolver().getLazyResolutionProxyIfNecessary(
					descriptor, requestingBeanName);
			if (result == null) {
				// requestingBeanName 进行依赖注入的 bean
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			// 如果当前 descriptor 做过依赖注入了，则直接使用 shortcut，相当于缓存
			...
			// 如果 descriptor 对应的类型是 数组 map 等 匹配该类型所有 bean
			Object multipleBeans = resolveMultipleBeans(descriptor, beanName, autowiredBeanNames, typeConverter);
			if (multipleBeans != null) {
				return multipleBeans;
			}

			// 根据类型找到所有 Bean，key 是 beanName,value 有可能是 bean 对象，有可能是 beanClass
			Map<String, Object> matchingBeans = findAutowireCandidates(beanName, type, descriptor);
			if (matchingBeans.isEmpty()) {
				// required 为 true,抛出异常
			...
			if (matchingBeans.size() > 1) {
				// 根据类型找到多个 bean，进行筛选 @Primary -->优先级最高 --》name
	...
	private Object resolveMultipleBeans(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		if (type.isArray()) {
			...
			// 如果 map 的 key 不是 string，key 保存的是 beanName
	...
	protected Map<String, Object> findAutowireCandidates(
		...
		// 从 BeanFactory 中找出和 requiredType 匹配的 beanName,这些bean不一定经过了实例化
		String[] candidateNames = BeanFactoryUtils.beanNamesForTypeIncludingAncestors(
				this, requiredType, true, descriptor.isEager());
		Map<String, Object> result = new LinkedHashMap<>(candidateNames.length);

		// 根据类型从 resolvableDependencies 中匹配 Bean , resolvableDependencies 中存放的是 类型：Bean 对象 （spring 启动时加入的）
		...
		for (String candidate : candidateNames) {
			// 如果不是自己，则判断该 candidate 能不能用来自动注入,优先注入别人
			// @Bean(autowireCandidate = false)
			if (!isSelfReference(beanName, candidate) && isAutowireCandidate(candidate, descriptor)) {
				addCandidateEntry(result, candidate, descriptor, requiredType);
			}
		}
		// 为空要么是真的没有匹配到。要么是匹配自己
	...
	protected String determineAutowireCandidate(Map<String, Object> candidates, DependencyDescriptor descriptor) {
		...
		// 判断找到的 Bean 中是否有一个被 @Primary 注解，并判断是否有多个被 @Primary 注解
		String primaryCandidate = determinePrimaryCandidate(candidates, requiredType);
		if (primaryCandidate != null) {
			return primaryCandidate;
		}

		// 取优先级最高的 Bean   @Priority(5)  数字越小优先级越高,只能注解类，最高优先级存在多个则报错
		String priorityCandidate = determineHighestPriorityCandidate(candidates, requiredType);
		if (priorityCandidate != null) {
			return priorityCandidate;
		}
		// Fallback
		// 匹配 descriptor 的名字，要么是字段的名字，要么是方法入参的名字
		for (Map.Entry<String, Object> entry : candidates.entrySet()) {
			String candidateName = entry.getKey();
			Object beanInstance = entry.getValue();

			// resolvableDependencies 记录了某个类型对应某个 bean
			// spring 自带的 byType descriptor.getDependencyName() 返回空
